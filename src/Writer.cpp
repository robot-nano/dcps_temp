...
Writer::Writer(::DDS::DataWriter_ptr writer_ptr)
    : writer_var_(::DDS::DataWriter::_duplicate(writer_ptr)),
...
void Writer::start() {
  ...
  // Each thread writers one instance which uses the thread id as the
  // key value
  if (activate(THR_NEW_LWP | THR_JOINABLE, num_instances_per_writer) == -1) {
    std::cerr << "Writer::start(): activate failed" << std::endl;
    exit(1);
  }
}
...
int Writer::svc() {
  ...
  try {
    while (1) {
      writer_var_->get_matched_subscriptions(handles);
      if (handles.length() > 0)
        break;
      else
    ...
    Messenger::MessageDataWriter_var message_dw
      = Messenger::MessageDataWriter::_narrow(writer_var_.in());
    if (CORBA::is_nil(message_dw.in())) {
      std::cerr << "Data Writer could not be narrowed" << std::endl;
      exit(1);
    }
    ...
    std::cerr << "Exception caught in svc: " << std::endl << e << std::endl;
  }
